...
class Database:
    ...
    async def add_filter(
        ...
        async with self._db.execute(
            """INSERT INTO filters
               (guild_id, channel_id, name, search_text, min_price, max_price,
                brand_ids, size_ids, catalog_ids, condition_ids, domain)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (guild_id, channel_id, name, search_text, min_price, max_price,
             brand_ids, size_ids, catalog_ids, condition_ids, domain),
        ) as cursor:
    ...
    async def has_any_seen(self, filter_id: int) -> bool:
        """True if at least one item has been marked seen for this filter."""
        async with self._db.execute(
